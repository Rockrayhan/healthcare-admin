  ...
  <body>
    <div x-data="setup()" x-init="$refs.loading.classList.add('hidden'); setColors(color);" :class="{ 'dark': isDark}">
      ...
      >
        ...
        >
          Welcome Editor
          {{-- actual admin table (created with middlewire)--}}
        </a>
        <main>
          @if ($errors->any())
      ...
      @endif
          <div class="w-full max-w-sm px-4 py-6 space-y-6 bg-white rounded-md dark:bg-darker">
            <h1 class="text-xl font-semibold text-center">Editor's Login</h1>
            <form class="space-y-6" method="POST" action="{{ route('adminLogin') }}">
              @csrf
              <input
